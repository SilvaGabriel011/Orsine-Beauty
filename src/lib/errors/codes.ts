/**
 * Modulo de Codigos de Erro — Bela Orsine Beauty
 *
 * Catalogo centralizado e tipado de erros da aplicacao.
 * Single source of truth: mensagens e status codes definidos uma unica vez.
 *
 * Convencao de nomenclatura:
 *   AUTH_xxx  → autenticacao / autorizacao
 *   VAL_xxx   → validacao de dados
 *   RES_xxx   → recurso generico (CRUD)
 *   APPT_xxx  → agendamentos
 *   REV_xxx   → avaliacoes
 *   PAY_xxx   → pagamentos
 *   LOY_xxx   → fidelidade (pontos)
 *   INT_xxx   → integracoes externas (Google Calendar, Email, WhatsApp)
 *   SYS_xxx   → erros de sistema / infraestrutura
 *   GAME_xxx  → gamificacao (minigames, moedas)
 *
 * Organizados por categoria para facilitar manutencao.
 */

export type ErrorCode = keyof typeof ERROR_CATALOG;

export interface ErrorDef {
  /** Mensagem amigavel para exibir ao usuario */
  message: string;
  /** HTTP status code correspondente */
  status: number;
}
